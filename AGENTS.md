...
- Use the repository's existing human Git identity when creating commits. Do not change
  `user.name` or `user.email` to an agent or bot identity.
- Never identify Claude, Claude Code, Codex, OpenAI, Anthropic, or another coding agent as
  a commit author, committer, co-author, contributor, or signatory.
- Do not add `Co-authored-by`, `Contributed-by`, `Signed-off-by`, `Generated-by`, or similar
  commit-message trailers or attribution text that names a coding agent.
- Keep commit subjects and bodies focused on the change itself; do not add statements such
  as "generated with Claude" or "generated by Codex".
